...
import com.ssafy.voyage.auth.dto.SignInAPIRequestDto;
...
public class MemberAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException {
        ...
        SignInAPIRequestDto signInAPIRequest = objectMapper.readValue(StreamUtils.copyToString(request.getInputStream(), StandardCharsets.UTF_8), SignInAPIRequestDto.class);

        String email = signInAPIRequest.getEmail();
        String password = signInAPIRequest.getPassword();

        if (email.isBlank() || password.isBlank()) {
            throw new AuthenticationException(MessageMaker.getMessageMaker().add(EMAIL).add(EMPTY).toString()) {
                @Override
                public String getMessage() {
                    return super.getMessage();
                }
